...
4. **중요도 도트** — 우상단, 작은 원 5개 (채워진 수 = 중요도 레벨)
...
카드 클릭 시 중앙 팝업 모달:
- 배경: `#fff`, 모서리 16px
- 구성: 카테고리 → 키워드(30px) → 영문명 → 중요도 바(가로 pip 5개) → 본문 → 태그 → 관련 키워드
- 오버레이: 반투명 베이지 blur
- ESC 또는 오버레이 클릭으로 닫기
...
키워드 등록 시 **1회 평가**. 트렌드가 아닌 AI 생태계 내 개념의 근본적 중요성.

### 5단계
| 레벨 | 라벨 | 의미 |
|-------|---------|------|
| 5 | 필수 | AI 개발/연구 전반에서 반드시 알아야 할 핵심 개념 |
| 4 | 중요 | 현재 AI 생태계에서 매우 활발히 쓰이는 개념 |
| 3 | 보통 | 특정 영역에서 널리 쓰이거나 성장 중인 개념 |
| 2 | 기초 | 기반 기술 또는 특수 목적용 개념 |
| 1 | 참고 | 학술적·역사적 의미가 있으나 실무 쓰임이 적은 개념 |

### 평가 방법
importance-analyst 에이전트가 등록 시 다중 소스(GeekNews MCP, GitHub, arXiv, Stack Overflow)를 조사하고 Claude의 종합 판단으로 결정. 이후 갱신 없음.
...
| 작업 | 사용 에이전트 |
|------|-------------|
| 키워드 추가 | researcher + reference-collector + importance-analyst |
| 기존 항목 수정 | researcher(검증) + reference-collector(최신 자료) |
| 트렌드 조사 | importance-analyst + researcher |
...
│   └── importance-analyst.md      # 다중 소스로 중요도 산출
├── hooks/
...
/add-keyword "Agentic RAG"
  ...
  ├─ [병렬] Agent(importance-analyst.md)  → 다중 소스 → 중요도
  │
